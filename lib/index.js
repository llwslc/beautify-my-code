...
module.exports = {
  ...
  {
    var self = this;
    atom.commands.add(`atom-workspace`, `${self.packageName}:format_on_save`, () => self.formatOnSave());
  ...
  formatOnSave: function ()
  {
    ...
    {
      atom.config.set(this.configSaveStr, `false`);
      selfPkgMenu.submenu[0].label = `Format on save`;
    }
    else
    {
      atom.config.set(this.configSaveStr, `true`);
      selfPkgMenu.submenu[0].label = `√ Format on save`;
    }
